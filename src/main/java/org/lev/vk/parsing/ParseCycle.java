...
import org.lev.ApplicationMain;
...
public class ParseCycle {
    ...
    public void compareDates(){
        ...
        if (group.getCurrentPostDate() == null){
            logger.info("First 'date' came in;");
            group.setCurrentPostDate(newPostDate);
        } else if (group.getCurrentPostDate() < newPostDate) {
            logger.info("New 'date' came in, must send an alert;");
            new SendAlert(new DefaultBotOptions(), group.getName());
            group.setCurrentPostDate(newPostDate);
        }else if(group.getCurrentPostDate() > newPostDate) {
            logger.info("Post was deleted, rewrite 'date' field;");
            group.setCurrentPostDate(newPostDate);
        }
    }
